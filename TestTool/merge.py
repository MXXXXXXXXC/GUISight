...
def incorporate(img_path, compo_path, text_path, output_root, params,
                resize_by_height=None, show=False, wait_key=0, savename = 'result.jpg'):
    ...
    for compo in json.load(open(compo_path, 'r'))['compos']:
        if compo['class'] == 'Background':
            background = compo
            continue
        element = Element((compo['column_min'], compo['row_min'], compo['column_max'], compo['row_max'],compo['cluster']), compo['class'])
    ...
    compos_json = save_corners_json(output_root, background, compos_merged, org_resize.shape, save_name=savename.replace('.png','.json').replace('.jpg', '.json'))
    dissemble_clip_img_fill(pjoin(output_root, 'clips'), org_resize, compos_json)
    cv2.imwrite(pjoin(output_root, savename), board)
