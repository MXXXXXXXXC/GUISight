...
def component_detection(binary, min_obj_area,
    ...
    for i in range(0, row, step_h):
        for j in range(i % 2, column, step_v):
            if binary[i, j] == 255 and mask[i, j] == 0:
                ...
                # ret, fff = cv2.threshold(mask_copy, 0, 255, cv2.THRESH_BINARY)
                # cv2.imshow("fff",fff )
                # cv2.waitKey(0)
                ...
                # do not ignore small area (muxi modify)
                if component.width <= 3 or component.height <= 3:
                    continue

                if ff[0] < min_obj_area:
                    compos_small.append(component)
                    continue
    ...
    if rec_detect:
        ...
        return compos_all, compos_small
