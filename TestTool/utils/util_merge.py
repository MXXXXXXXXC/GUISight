...
def save_corners_json(output_dir, background, compos, img_shape, save_name='compo.json'):
    ...
    for i, compo in enumerate(compos):
        corner = compo.put_bbox()
        c = {'id': i, 'class': compo.category, 'height': corner[3] - corner[1], 'width': corner[2] - corner[0],
             'column_min': corner[0], 'row_min': corner[1], 'column_max': corner[2], 'row_max': corner[3],
             'cluster': compo.cluster}
        c_path = pjoin(clip_dir, c['class'], str(i) + '.jpg')
        c['clip_path'] = c_path
        components['compos'].append(c)

    json.dump(components, open(pjoin(output_dir, save_name), 'w'), indent=4)
